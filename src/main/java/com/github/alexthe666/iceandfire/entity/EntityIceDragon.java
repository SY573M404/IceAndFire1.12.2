...
public class EntityIceDragon extends EntityDragonBase {
    ...
    public boolean attackEntityAsMob(Entity entityIn) {
        ...
        if(!this.isPlayingAttackAnimation()){
            switch (groundAttack) {
                ...
                case SHAKE_PREY:
                    boolean flag = false;
                    if (new Random().nextInt(2) == 0 && isDirectPathBetweenPoints(this, this.getPositionVector().addVector(0, this.height/2, 0), entityIn.getPositionVector().addVector(0, entityIn.height/2, 0)) &&
    ...
    public void riderShootFire(Entity controller) {
        if (this.getRNG().nextInt(5) == 0 && !this.isChild()) {
            if (this.getAnimation() != ANIMATION_FIRECHARGE) {
                ...
                EntityDragonIceCharge entitylargefireball = new EntityDragonIceCharge(world, getRidingPlayer(), this, d2, d3, d4);
    ...
    public void stimulateFire(double burnX, double burnY, double burnZ, int syncType) {
        ...
        for (int i = 0; i < conqueredDistance; i += increment) {
            ...
            if (canPositionBeSeen(progressX, progressY, progressZ)) {
                ...
                if (!world.isRemote) {
                    ...
                    IafDragonDestructionManager.destroyAreaIce(world, getRidingPlayer(), pos, this);
                }
            }
        }
        if (burnProgress >= 40D && canPositionBeSeen(burnX, burnY, burnZ)) {
            ...
            if (!world.isRemote) {
                IafDragonDestructionManager.destroyAreaIce(world, getRidingPlayer(), new BlockPos(spawnX, spawnY, spawnZ), this);
            }
        }
    }
